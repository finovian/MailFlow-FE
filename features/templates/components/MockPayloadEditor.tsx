...
export default function MockPayloadEditor({
  ...
  const suggestedVariables = React.useMemo(() => {
    return unknownVariables
      .map((unk) => {
        const bestMatch = availableFields.find(
          (f) =>
            f.includes(unk) ||
            unk.includes(f) ||
            f.split(".").pop() === unk.split(".").pop(),
        );
  ...
  return (
    <div className="flex flex-col gap-4 rounded-lg border border-border/50 bg-card p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-3">
          <div className="space-y-2">
            <div className="flex items-center gap-1.5 text-xs font-bold text-green-600 dark:text-green-500 uppercase tracking-wider">
            ...
            {usedVariables.length > 0 ? (
              <div className="flex flex-wrap gap-1">
                {usedVariables.map((v) => (
                  <span
                    key={v}
                    className="rounded bg-green-500/10 px-1.5 py-0.5 text-xs font-mono text-green-600 dark:text-green-500 border border-green-500/20"
                  >
                    {v}
                  </span>
                ))}
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1">
                {unknownVariables.map((v) => (
                  <span
                    key={v}
                    className="rounded bg-destructive/10 px-1.5 py-0.5 text-xs font-mono text-destructive border border-destructive/20"
                  >
                    {v}
                  </span>
                ))}
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1">
                {suggestedVariables.map((s) => (
                  ...
                  >
                    <span className="font-mono text-destructive/70 line-through">
                      {s.unknown}
                    </span>
                    <span>→</span>
                    <span className="font-mono text-amber-600 dark:text-amber-500 font-semibold">
                      {s.suggestion}
                    </span>
                  </div>
                ))}
            ...
            {unusedFields.length > 0 ? (
              <div className="flex flex-wrap gap-1 opacity-70">
                {unusedFields.map((f) => (
                  <span
                    key={f}
                    className="rounded bg-muted px-1.5 py-0.5 text-xs font-mono text-muted-foreground border border-border/50"
                  >
                    {f}
                  </span>
                ))}
                ...
                {variables.map((variable) => (
                  <div key={variable} className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleFieldChange(variable, e.target.value)
                      }
