...
import {
  useGenerateTrigger,
  useFixTemplateVariables,
  useSuggestConditions,
} from "@/features/triggers/hooks/useTriggerAi";
...
export function TriggerForm({ initialData }: TriggerFormProps) {
  ...
  const [customMockPayload, setCustomMockPayload] = React.useState<
    Record<string, any>
  >({});
  ...
  const [fixedHtmlContent, setFixedHtmlContent] = React.useState<string | null>(
    null,
  );
  ...
  // AI helper hooks
  const { mutate: generateTriggerWithAi, isPending: isGeneratingAi } =
    useGenerateTrigger();
  const { mutate: fixVariablesWithAi, isPending: isFixingVariables } =
    useFixTemplateVariables();
  const { mutate: suggestConditionsWithAi, isPending: isSuggestingConditions } =
    useSuggestConditions();

  // Mapped templates options for display
  const eventDefinitionsGrouped = React.useMemo(
    () => getEventDefinitionsGrouped(),
    [],
  );
  ...
  const handleEventTypeChange = (newVal: any) => {
    ...
    const emailField =
      fields.find((f) => f.name.toLowerCase().includes("email"))?.name || "";
    if (emailField) {
    ...
    if (!triggerName) {
      const label =
        eventOptions.find((e) => e.value === newVal)?.label || newVal;
    ...
    generateTriggerWithAi(
      ...
      },
      {
        onSuccess: (data) => {
          ...
          if (data.recipientField)
            setValue("recipientField", data.recipientField);
          if (data.cooldownDays !== undefined)
            setValue("cooldownDays", data.cooldownDays);
        ...
        onError: (err: any) => {
          toast.error(
            err?.message ||
              "AI failed to configure the trigger. Please try manually.",
          );
        },
      },
    );
  ...
  const [suggestedConditions, setSuggestedConditions] = React.useState<
    Array<{ label: string; conditions: ConditionGroup }>
  >([]);
  const handleSuggestConditions = () => {
    if (!eventType) return;

    toast.info(
      "AI is analyzing event properties to generate condition filters...",
    );
    ...
    <div className="flex flex-col gap-6 max-w-7xl mx-auto px-4 md:px-6 py-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border/40 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-medium leading-tight tracking-tight sm:text-3xl text-foreground flex items-center gap-2">
              {isEdit
                ? `Edit Trigger: ${initialData.name}`
                : "Create Automation Trigger"}
            </h1>
            <p className="text-xs text-muted-foreground mt-0.5">
              Choose when emails should be sent automatically based on system
              events
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          {!isEdit && (
            <Button
              ...
              className={cn(
                "h-10 text-sm gap-2 font-medium transition-all",
                showAiPrompt
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                className="gap-1.5 text-sm h-10 bg-indigo-600 hover:bg-indigo-500 text-white font-medium"
              >
                {isGeneratingAi ? (
            ...
            <p className="text-xs text-muted-foreground">
              AI will automatically select the event type, recommended template,
              map variables, and create conditional filters.
            </p>
          </div>
        </Card>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <Card className="bg-card border-border/50 shadow-xs transition-all duration-200">
                ...
                >
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      className="w-full justify-between h-10 text-sm font-medium bg-card/60 shadow-none border-border/80"
                    >
                      {eventType ? (
                        <div className="flex items-center gap-2">
                          <Zap className="size-3.5 text-amber-500 fill-amber-500/10" />
                          <span className="font-semibold text-foreground">
                            {eventOptions.find((e) => e.value === eventType)
                              ?.label || eventType}
                          </span>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {eventFields.map((f) => (
                      <Badge
                        key={f.name}
                        variant="secondary"
                        className="font-mono text-xs px-2 py-0.5 bg-card border border-border/40 text-muted-foreground font-normal"
                      >
                        {f.name}{" "}
                        <span className="opacity-60 text-xs">({f.type})</span>
                      </Badge>
                    ))}
                  </div>
          ...
          {showStep2 && (
            <Card className="bg-card border-border/50 shadow-xs transition-all duration-200 animate-in slide-in-from-bottom-2 duration-300">
              <CardHeader className="pb-4">
                <div className="flex items-center gap-2">
                  <div className="flex size-7 items-center justify-center rounded-lg bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-xs font-bold font-mono">
                    2
                    ...
                    {!isTemplateCompatible ? (
                      <div className="p-3.5 bg-rose-500/10 border border-rose-500/20 rounded-lg flex flex-col md:flex-row md:items-center justify-between gap-3 animate-in shake duration-300">
                        <div className="flex gap-2">
                          ...
                          <div className="space-y-0.5">
                            ...
                            <div className="text-xs text-rose-600 dark:text-rose-300 leading-normal">
                              Template requires:{" "}
                              <code className="bg-rose-500/10 px-1 py-0.5 rounded font-mono text-xs">
                                {extraVars.join(", ")}
                              </code>{" "}
                        ...
                        <Button
                          ...
                          className="h-8 border-rose-300/40 text-rose-700 hover:bg-rose-500/10 dark:text-rose-300 text-xs gap-1 font-semibold"
                        >
                          {isFixingVariables ? (
                      ...
                      <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg flex items-center gap-2 text-emerald-700 dark:text-emerald-400">
                        ...
                        <span className="text-xs font-semibold">
                          All template variables are compatible with the
                          selected event.
                        </span>
                      </div>
                    )}
                        ...
                        {eventFields
                          ...
                          .map((f) => (
                            <button
                              ...
                              className={cn(
                                "flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs border transition-all cursor-pointer",
                    ...
                    <div>
                      <h3 className="text-sm font-semibold flex items-center gap-1.5">
                        ...
                        {flatRulesList.length > 0 && (
                          <Badge
                            variant="secondary"
                            className="px-1.5 py-0 text-xs bg-indigo-500/10 text-indigo-600 border-indigo-200/20"
                          >
                      ...
                      <p className="text-xs text-muted-foreground">
                        Define conditional logic rules before automated
                        execution occurs.
                      </p>
                    </div>
                  </div>
                  ...
                  <CardContent className="border-t border-border/30 pt-4 space-y-4 animate-in fade-in-50 duration-200">
                    <div className="flex flex-wrap items-center justify-between gap-3 bg-indigo-500/5 dark:bg-indigo-950/10 border border-indigo-100/30 dark:border-indigo-950/20 rounded-lg p-3">
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Generate conditional logic recommended for the "
                          {eventType}" event.
                        </p>
                      </div>
                      <Button
                        ...
                        className="h-8 text-xs border-indigo-200/40 text-indigo-600 dark:text-indigo-400 gap-1 bg-card hover:bg-indigo-500/10 font-semibold"
                      >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-2 pb-2">
                        {suggestedConditions.map((s, idx) => (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              applySuggestedConditions(s.conditions)
                            }
                ...
                >
                  <div className="flex items-center gap-2 text-left">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Configure status, deduplication triggers, and execution
                        cooldowns.
                      </p>
                    </div>
                  </div>
                ...
                {advancedExpanded && (
                  <CardContent className="border-t border-border/30 pt-4 space-y-4 animate-in fade-in-50 duration-200">
                    <div className="space-y-1.5">
                      ...
                      />
                      <p className="text-xs text-muted-foreground">
                        Wait period (days) required before triggers will
                        evaluate for the same recipient again.
                      </p>
                      {errors.cooldownDays && (
                        <p className="text-xs font-medium text-destructive">
                          {errors.cooldownDays.message}
                        </p>
                      )}
                    </div>
            ...
            <CardContent className="p-4 min-h-[420px] flex flex-col justify-between">
                    ...
                    <div className="p-3 bg-amber-500/5 border border-amber-500/10 rounded-xl relative group">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="space-y-0.5 text-left">
                          ...
                          <span className="text-xs font-semibold text-foreground">
                            {eventType
                              ? `When ${eventOptions.find((e) => e.value === eventType)?.label || eventType} occurs`
                            ...
                            <div className="space-y-1 pt-0.5">
                              ...
                              <div className="space-y-1">
                                {flatRulesList.map((r, i) => (
                                  <div
                                    key={i}
                                    className="flex items-center gap-1.5 text-xs text-muted-foreground font-mono bg-card border border-border/30 px-2 py-0.5 rounded"
                                  >
                    ...
                    <div className="p-3 bg-indigo-500/5 border border-indigo-500/10 rounded-xl">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="space-y-0.5 text-left">
                          ...
                          <span className="text-xs font-semibold text-foreground">
                            {templateId
                              ? `Send template: ${templates.find((t) => t.id === templateId)?.name || templateId}`
                  ...
                  {!templateId ? (
                    <div className="flex flex-col items-center justify-center gap-2 text-muted-foreground py-16 flex-1">
                      ...
                      <span className="text-xs">
                        Select a template to render email mockup.
                      </span>
                    </div>
                  ) : (
                    <div className="flex-1 min-h-[360px] border border-border/40 rounded-xl overflow-hidden bg-white shadow-inner flex flex-col">
                      <div className="flex items-center gap-1.5 border-b border-border/50 bg-muted/40 px-3 py-1.5">
                        ...
                        <div className="mx-auto text-xs font-semibold text-muted-foreground font-mono">
                          Live Render Sandbox
                        </div>
                      </div>
                      <iframe
              ...
              {previewTab === "mock" && (
                <div className="flex-1 flex flex-col gap-2 py-2">
                  ...
                  <textarea
                    ...
                    className="w-full h-80 font-mono text-xs p-3 bg-zinc-950 text-green-400 rounded-xl border border-border focus:outline-none focus:ring-1 focus:ring-indigo-500 resize-none leading-relaxed"
                  />
                </div>
              )}
              ...
              <div className="mt-4 pt-3 border-t border-border/40 flex items-center justify-between text-xs text-muted-foreground font-medium">
                <div className="flex items-center gap-1">
                  ...
                  <span>
                    Cooldown: {cooldownDays > 0 ? `${cooldownDays}d` : "None"}
                  </span>
                </div>
                <div>
