...
export function ConditionBuilder({
  ...
  const [openSelectors, setOpenSelectors] = React.useState<
  ...
  const updateTree = (
    ...
    updatedRules[index!] = updateTree(
      updatedRules[index!] as ConditionGroup,
      rest,
      updateFn,
    );
  ...
  const handleUpdateRule = (
    path: number[],
    index: number,
    updatedRule: Partial<ConditionRule>,
  ) => {
    const next = updateTree(value, path, (group) => {
    ...
    return (
      <div
        className={cn(
          ...
          isRoot ? "bg-card border-border/80" : "bg-muted/10 ml-4 md:ml-6",
        )}
      >
        {/* Header toolbar */}
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <Select
              ...
              onValueChange={(val: "AND" | "OR") =>
                handleUpdateGroupOperator(path, val)
              }
            >
              <SelectTrigger
                className="h-7 w-[80px] text-xs font-bold bg-card"
                size="sm"
              >
                <SelectValue />
              </SelectTrigger>
          ...
          <div className="flex items-center gap-1.5">
            <Button
              ...
              variant="outline"
              size="xs"
              className="h-7 text-xs gap-1 cursor-pointer font-medium bg-card"
                  ...
                  <CommandGroup heading="Event Schema properties">
                    {fields.map((f) => (
                      ...
                      >
                        ...
                        <span className="text-xs text-muted-foreground font-sans bg-muted/40 px-1 py-0.5 rounded border border-border/10 shrink-0">
                          {f.type}
                        </span>
                      </CommandItem>
                    ))}
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between pb-1 border-b border-border/40">
        <Label className="text-xs font-semibold text-foreground">
          Rule Filter Hierarchy
        </Label>
        <span className="text-xs text-muted-foreground font-medium hidden sm:inline">
          Evaluated against the incoming{" "}
          <code className="rounded bg-muted/80 border border-border/30 px-1 py-0.2 font-mono text-xs">
