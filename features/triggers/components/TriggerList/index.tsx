...
export function TriggerList() {
  ...
  const [simulateTrigger, setSimulateTrigger] = React.useState<Trigger | null>(
    null,
  );
  ...
  const { mutate: fireEvent, isPending: isFiringEvent } = useMutation({
    ...
    onSuccess: () => {
      toast.success(
        "Simulation event fired! Triggers are being evaluated in the background.",
      );
  ...
  const filteredData = React.useMemo(() => {
    ...
    return data.data.filter((item) => {
      ...
      const matchesEventType =
        eventTypeFilter === "all" || item.eventType === eventTypeFilter;
        ...
        cell: (row) => (
          ...
          >
            ...
            <Button
              ...
              size="icon-xs"
              title={
                row.status === "active" ? "Pause Trigger" : "Activate Trigger"
              }
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-40">
                <DropdownMenuItem
                  onClick={() => router.push(`/triggers/${row.id}`)}
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <Button onClick={() => router.push("/triggers/new")} className="gap-2">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                >
                  <SelectTrigger
                    className="h-8.5 text-xs w-[130px] bg-card/65 shadow-none"
                    size="sm"
                  >
              ...
              >
                <SelectTrigger
                  className="h-8.5 text-xs w-[170px] bg-card/65 shadow-none"
                  size="sm"
                >
      ...
      {!isLoading && filteredData.length === 0 && (
        <Card className="border-dashed border-border/80 bg-muted/5 py-12 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-xs text-muted-foreground max-w-xs mt-1">
            Try adjusting your search queries or filters, or create your first
            automation flow.
          </p>
        </Card>
      )}
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          variant="secondary"
                          className="px-1.5 py-0 text-xs bg-indigo-500/10 text-indigo-600 border-indigo-200/10 dark:text-indigo-400"
                        >
      ...
      {/* Simulation Dialog */}
      <Dialog
        open={!!simulateTrigger}
        onOpenChange={(open) => !open && setSimulateTrigger(null)}
      >
        <DialogContent className="sm:max-w-[520px]">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              Dispatch a test event payload of type "
              {simulateTrigger?.eventType}" to trigger rules evaluation.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            <div className="rounded-lg bg-indigo-50/40 dark:bg-indigo-950/10 border border-indigo-100/40 dark:border-indigo-950/20 p-3 space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs font-medium text-muted-foreground leading-relaxed">
                <div>
                  Name:{" "}
                  <span className="text-foreground">
                    {simulateTrigger?.name}
                  </span>
                </div>
                <div>
                  Event:{" "}
                  <span className="font-mono text-foreground">
                    {simulateTrigger?.eventType}
                  </span>
                </div>
                <div>
                  Template:{" "}
                  <span className="text-foreground">
                    {simulateTrigger?.templateName ||
                      simulateTrigger?.templateId}
                  </span>
                </div>
                <div>
                  Recipient:{" "}
                  <span className="font-mono text-foreground">
                    {simulateTrigger?.recipientField}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                Edit the JSON payload properties to verify conditions criteria
                and variable merges correctly.
              </p>
            </div>
          </div>
          ...
          <DialogFooter className="pt-2">
            ...
            <Button
              ...
              disabled={isFiringEvent}
              className="text-sm h-10 bg-indigo-600 hover:bg-indigo-500 text-white font-medium gap-1.5"
            >
              {isFiringEvent ? (
