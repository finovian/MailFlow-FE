...
export function AIAssistPanel({
  ...
  const [activeTab, setActiveTab] = React.useState<
    "subject" | "tone" | "validate"
  >("subject");

  const {
    mutate: getVariants,
    isPending: loadingVariants,
    data: variants,
  } = useSubjectVariants();
  ...
  const {
    mutate: rewriteTone,
    isPending: loadingTone,
    data: rewrittenResult,
  } = useToneRewrite();
  ...
  return (
    <div className="flex h-full flex-col rounded-xl border border-border/50 bg-card p-4 backdrop-blur-xl">
      ...
      >
        ...
        <TabsContent
          value="subject"
          className="flex flex-1 flex-col gap-3 mt-3"
        >
          ...
          {variants && variants.length > 0 && (
            <div className="space-y-2 mt-2 flex-1 overflow-y-auto max-h-[320px] pr-1">
              ...
              {variants.map((v: any, i: number) => (
                ...
                >
                  <p className="text-xs font-semibold text-foreground pr-8">
                    {v.subject}
                  </p>
                  <span className="mt-1 inline-block rounded bg-primary/10 px-1 py-0.5 text-xs uppercase tracking-wider text-primary font-medium">
                    {v.tone}
                  </span>
            ...
            <div className="rounded-lg border border-border/40 bg-muted/20 p-2.5 mt-2">
              ...
              <div className="flex justify-end gap-1.5 mt-2">
                ...
                <Button
                  ...
                  onClick={() =>
                    onApplyHtml(
                      rewrittenResult.rewritten.replace(/\n/g, "<br />"),
                    )
                  }
                >
                  Insert HTML
            ...
            <div className="space-y-3 mt-2 flex-1 overflow-y-auto max-h-[300px] pr-1">
              ...
              {validationResult.valid && validationResult.valid.length > 0 && (
                <div className="space-y-1">
                  <div className="flex items-center gap-1 text-xs font-semibold text-green-600 dark:text-green-500">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {validationResult.valid.map((v: string) => (
                      <span
                        key={v}
                        className="rounded bg-green-500/10 px-1.5 py-0.5 text-xs font-mono text-green-600 dark:text-green-500"
                      >
                        {v}
                      </span>
                    ))}
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {validationResult.missing.map((v: string) => (
                        <span
                          key={v}
                          className="rounded bg-amber-500/10 px-1.5 py-0.5 text-xs font-mono text-amber-600 dark:text-amber-500"
                        >
                          {v}
                        </span>
                      ))}
              ...
              {validationResult.extra && validationResult.extra.length > 0 && (
                <div className="space-y-1">
                  <div className="flex items-center gap-1 text-xs font-semibold text-destructive">
                    <AlertCircle className="size-3" /> Unknown Variables (Will
                    Fail)
                  </div>
                  <div className="flex flex-wrap gap-1">
                    {validationResult.extra.map((v: string) => (
                      <span
                        key={v}
                        className="rounded bg-destructive/10 px-1.5 py-0.5 text-xs font-mono text-destructive"
                      >
                        {v}
                      </span>
                    ))}
