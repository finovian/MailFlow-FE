  ...
  return (
    <div className="mt-3 rounded-lg border border-border/40 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-2">
        ...
        {triggerName && (
          <span className="text-xs font-semibold text-primary bg-primary/5 px-1.5 py-0.5 rounded border border-primary/10">
            {triggerName}
          </span>
        )}
      </div>
      ...
      <div className="pt-2 border-t border-border/10 flex items-center justify-between">
        <span className="text-xs text-muted-foreground italic">Result</span>
        <span
          className={cn(
...
export function EventTimeline({ event }: EventTimelineProps) {
  ...
  const timelineSteps: TimelineStepData[] = (parsedError?.timeline ||
    apiTimeline ||
    []) as TimelineStepData[];
        ...
        <div className="flex flex-col gap-1 border-b border-border/20 pb-4">
          <div className="flex items-center justify-between">
            ...
            {status === "PENDING" || status === "PROCESSING" ? (
              <div className="flex items-center gap-1.5 text-xs font-bold text-primary bg-primary/5 border border-primary/10 rounded-full px-2.5 py-1 animate-pulse">
                <Loader2 className="size-3 animate-spin" />
                <span>Workflow running...</span>
              </div>
            ) : (
              <div className="text-xs font-bold text-muted-foreground bg-muted/30 border border-border/40 rounded-full px-2.5 py-1">
                Completed
              </div>
            )}
          </div>
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center space-y-3">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground max-w-[200px]">
              The automation engine is currently executing workflow rules for
              this event.
            </p>
          </div>
        </div>
