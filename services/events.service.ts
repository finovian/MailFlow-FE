  ...
  getAll: async () => {
    try {
      const response = await apiClient.get<{
        success: boolean;
        data: { items: EventApiResponse[] };
      }>("/events");
      const items: SimulationEvent[] = (response.data.data.items || []).map(
        (item) => ({
          id: item.id,
  ...
  getById: async (id: string): Promise<SimulationEvent> => {
    ...
    try {
      ...
      jobs = allJobs
        .filter(
          (j) =>
            j.eventId === id || (j.event as Record<string, unknown>)?.id === id,
        )
        .map((j) => ({
          id: j.id as string,
