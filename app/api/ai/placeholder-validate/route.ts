  ...
  try {
    ...
    if (!html || !eventType) {
      return NextResponse.json(
        { error: "HTML and eventType are required" },
        { status: 400 },
      );
    }

    const availableFields = getFieldsForEventType(eventType as EventType).map(
      (f) => f.name,
    );
...
- 'extra': placeholders found in the HTML that are NOT in the available variables.`,
    ...
    return NextResponse.json(
      { error: error.message || "Failed to validate placeholders" },
      { status: 500 },
    );
  }
}
