...
> = {
  text: (OPERATORS as readonly OperatorDescriptor[]).filter(
    (op) => op.inputType === "text",
  ),
  number: (OPERATORS as readonly OperatorDescriptor[]).filter(
    (op) => op.inputType === "number",
  ),
  boolean: (OPERATORS as readonly OperatorDescriptor[]).filter(
    (op) => op.inputType === "boolean",
  ),
  none: (OPERATORS as readonly OperatorDescriptor[]).filter(
    (op) => op.inputType === "none",
  ),
} as const;
